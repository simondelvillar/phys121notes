...
\begin{tcolorbox}[
  title={Recent Updates \hfill Last updated: 19/1/26 2:30PM},
  colback=gray!5,
  colframe=black!40,
  boxrule=0.5pt,
  arc=4pt,
...
\begin{itemize}
  ...
  \item \textbf{Jan 19:} Added \changelink{sec:lecture6-long-rod-ring}{(In progress) Lecture 6 - "Infinitely" long rod and "finite" ring}  

  \item \textbf{Jan 17:} Added \changelink{sec:lecture5-electric-field}{Lecture 5 - Electric Field}  

  \item \textbf{Jan 14:} Added \changelink{sec:lecture4-coulombs-law}{Lecture 4 - Coulomb's Law}
  \subitem  Added \changelink{sec:lecture4.1-vector-form}{Lecture 4 - The Vector Form of Coulomb's Law}
...
\begin{center}
    ...
    \input{figures/equilibrium-spring.tikz}
\end{center}
...
\begin{center}
    ...
    \input{figures/extended-spring.tikz}
\end{center}
...
\begin{center}
    ...
    \input{figures/compressed-spring.tikz}
\end{center}
...
\begin{center}
    \input{figures/small-angle-approx1.tikz}
\end{center}
...
\begin{center}
    \input{figures/small-angle-approx-zoom1.tikz}
\end{center}
...
\begin{center}
    \input{figures/atoms-in-box.tikz}
\end{center}

\noindent Bringing a charged object:

\begin{center}
    \input{figures/polarized-box.tikz}
\end{center}

\begin{center}
    \input{figures/polarized-box2.tikz}
\end{center}
...
\begin{center}
    \input{figures/coulombs-law.tikz}
\end{center}
...
\begin{center}
    \input{figures/example1.1solution.tikz}
\end{center}
...
\begin{center}
    \input{figures/coulombslawvector.tikz}
\end{center}
...
\label{sec:lecture5-electric-field}
...
\begin{center}
    \includegraphics[scale=1.5]{figures/efieldlinesneg.pdf}
\end{center}

\noindent \textbf{Core Concept.} Electric field lines originate from positive charges and terminate on negative charges. 


\section{"Infinitely" long rod and "finite" ring}
\label{sec:lecture6-long-rod-ring}

\noindent Calculate the electric field due to a uniformly charged rod that is infinitely long.

\vspace{1.5em}

\begin{center}
    \includegraphics[scale=1.5]{figures/rodsetup.pdf}
\end{center}

\noindent We can select a section of rod of length $\Delta x$ that is small. That section has a individual charge $\Delta q$ which we treat as a point charge.
\vspace{1em}

\noindent \textbf{Note:} For every $\Delta q$ to the right of the $y$-axis (in blue), there is a corresponding $\Delta q$ placed symmentrically on the left (in red).

\vspace{1em}

\noindent The pairs of charges create a $\displaystyle \vec{E}_{\text{net}}$ at $P$ with $x$-components that cancel and $y$-components that add. 

\vspace{1em}

\noindent By symmetry, the $\displaystyle \vec{E}_{\text { net }}$ points perpendicularly away from the rod, regardless of where P is placed. From the problem last Friday, we know that the contribution to $\vec{E}$ from $\Delta q$ along the $y$-axis is given by:
\begin{align*}
    \displaystyle \vec{E}_{\Delta q, y} = \dfrac{k_e \Delta q \, y}{\left(x^2+y^2\right)^{3/2}}
\end{align*}

\noindent This is the magnitude of the $\vec{E}$ contribution from a single $\Delta q$ along the $y$-axis. 

\vspace{1.5em}

\noindent The $\displaystyle\vec{E}_{\text {net}}$ due to the entire rod is found by summing up all the contributions from individual $\Delta q$'s that make up the charged rod:
\begin{align*}
    E_{\text{net}} = \sum_{i} \dfrac{k_e \Delta q \, y}{\left(x_i^{2}+y^2\right)^{3/2}}
\end{align*}

\noindent noting that subscript $i$ is representative of all the individual $\Delta q$'s.


\subsection{Linear Charge Density}

This is precisely charge per unit of length of the rod. This is given by the Greek letter "lambda":
\begin{align*}
    \lambda = \dfrac{Q}{L}
\end{align*}
where $Q$ is the total charge in a section of rod of length $L$.

\vspace{1.5em}

\noindent Therefore, a section of length $\Delta x$ has charge:
\begin{align*}
    \Delta q &= Q \dfrac{\Delta x}{L} \\
             &= \left(\frac{Q}{L}\right)\Delta x \\
             &= \lambda\Delta x \\
             &= \Delta q = \lambda\Delta x   
\end{align*}

\subsection{Lecture 6 continued}
With this in mind:
\begin{align*}
    E_{\text{net}} \approx \sum_{i} \dfrac{k_e \lambda \Delta x \, y}{\left(x_i^{2}+y^2\right)^{3/2}}
\end{align*}

\noindent This is a Riemann sum! In the limit as $\Delta x \to 0$, $\Delta q$'s become exact point charges:
\begin{align*}
    E_{\text{net}} &\approx \lim\limits_{\Delta x \to 0}\sum_{i} \dfrac{k_e \lambda \Delta x \, y}{\left(x_i^{2}+y^2\right)^{3/2}} \Delta x \\[6pt]
                   &\Rightarrow \int \dfrac{k_e \lambda \, y}{\left(x_i^{2}+y^2\right)^{3/2}} \, dx
\end{align*}

\noindent The term $dx$ represents \textit{infinitessimally} small chunks of $\Delta x$.

\vspace{1em}

\noindent We can "pull out" constants from the integral. In this case $k_e$ and $\lambda$ are constants:
\begin{align*}
    k_e \lambda\int \dfrac{y}{\left(x^2+y^2\right)^{3/2}} \, dx
\end{align*}

\noindent To evaluate this intergal, we will consider the following:
\begin{align*}
    \dfrac{d}{dx} \left(\dfrac{y}{\left(\sqrt{x^2+y^2}\right)}\right)
\end{align*}

\noindent Using the product and chain rules, $\dfrac{d}{dx}$ of this equation is:
\begin{equation}
    \dfrac{y^2}{\left(x^2+y^2\right)^{3/2}}
\end{equation}

\noindent Result (9) can be rewritten as the following, knowing that $y$ is a constant.
\begin{equation}
    \Rightarrow \dfrac{1}{y}\dfrac{d}{dx}\left[\dfrac{x}{\sqrt{x^2+y^2}}\right]
\end{equation}

\noindent We can take equation (10) and plug it into the original integral:
\begin{align*}
    E_{\text{net}} &= k_e \lambda \int \dfrac{1}{y}\dfrac{d}{dx}\left(\dfrac{x}{\sqrt{x^2+y^2}}\right) \, dx \\[6pt]
                   &\Rightarrow \dfrac{k_e \lambda}{y}\int \dfrac{d}{dx}\left(\dfrac{x}{\sqrt{x^2+y^2}}\right) \, dx
\end{align*}

\noindent\textbf{Major Calculus Theorem: Fundamental Theorem of Calculus} \textit{In simple words, an integral of the derivative of any function} $f(x)$ \textit{is the same as evaluating the functions at the bounds of integration.}
\begin{equation*}
    \int\limits_{a}^{b}\dfrac{df}{dx} = f(b)-f(a)
\end{equation*}

\noindent


\noindent Invoking \textbf{FTC}, we can recognize this form in our integral and apply it into the infinite rod problem: 
\begin{align*}
    E_{\text{net}} &= \dfrac{k_e \lambda}{y}\int\limits_{-\infty}^{\infty}\dfrac{d}{dx}\left(\dfrac{x}{\sqrt{x^2+y^2}}\right) \, dx\\[6pt]
                   &\Rightarrow \dfrac{k_e \lambda}{y} \left[\dfrac{\infty}{\sqrt{\infty^{2}+y^2}}-\dfrac{-\infty}{\sqrt{\infty^{2}+y^2}}\right]\\[6pt]
                   &\Rightarrow \dfrac{k_e \lambda}{y} \left[1-(-1)\right]
\end{align*}

\noindent Finally, the magnitude of $\vec{E}$ due to an uniformly charged rod that is infinite in length is:
\begin{align*}
    \vec{E}_{\text{net}} = \dfrac{2 k_e \lambda}{y}
\end{align*}

\subsection{Rings}
Let us try another continous distribution of charge. This time, a ring. 

\vspace{2em}

\noindent We need to find $\vec{E}$ at $P$ along the $z$-axis that passes through the centre of uniformly charged ring of radius $r$. By symmetry, $\vec{E}_{\text{net}}$ due to the charged ring will point along the $z$-axis.

\vspace{1em}

\noindent $\displaystyle \vec{E}_{\Delta q, z}$ can be found by the following:
\begin{align*}
    \vec{E}_{\Delta q, z} &= E_{\Delta q}\cos\theta\\
                          &= \dfrac{z}{\left(z^2+r^2\right)}
\end{align*}

Therefore, 
\begin{align*}
    \vec{E}_{\Delta q, z} = \dfrac{k_e \Delta q}{z^2+r^2} \dfrac{z}{z^2+r^2} = \dfrac{k_e \Delta q z}{\left(z^2+r^2\right)^{3/2}}
\end{align*}

noting that $\Delta q$, $z$, and $r$ are constants for any $\Delta q$ on the ring.

\vspace{1.5em}

\noindent Modelling after the previous problem: 
\begin{align*}
    E_{\text{net}} &= \sum_{i} E_{\Delta q, z} \\
                   &= \sum_{i} \dfrac{k_e \Delta q_{i} z}{\left(z^2+r^2\right)^{3/2}}\\
                   &= \dfrac{k_e z}{\left(z^2+r^2\right)^{3/2}} \sum_{i} \Delta q_{i}
\end{align*}

where $\displaystyle\sum_{i}\Delta q_{i}$ is the total charge of ring $Q$.
...
\begin{equation*}
    E_{\text{net}} = \dfrac{k_e Q z}{\left(z^2+r^2\right)^{3/2}}
\end{equation*}

\noindent pointing down along the $z$-axis for $Q<0$.
