...
\noindent Calculate the electrie field due to a uniformly charges road that is infinitely long.
...
\noindent \textbf{Note:} For every $\Delta q$ to the right of the $y$-axis (red), there is a corresponding $\Delta q$ placed symmentrically on the left (in blue).
...
This is the magnitude of the $\vec{E}$ contribution from a single $\Delta q$ along the $y$-axis. 



\vspace{2em}

\noindent The $\displaystyle\vec{E}_{\text {net}}$ due to the entire rod is found by summing all the contributions from individual $\Delta q$'s that make up the charged rod.
\begin{align*}
    E_{\text{net}} = \sum_{i} \dfrac{k_e \Delta q \, y}{\left(x_i^{2}+y^2\right)^{3/2}}
\end{align*}

where $i$ is all the individual $\Delta q$'s.
...
This is a Riemann sum! In the limit as $\Delta x \to 0$, $\Delta q$'s become exact point charges:
\begin{align*}
    E_{\text{net}} &\approx \sum_{i} \dfrac{k_e \lambda \Delta x \, y}{\left(x_i^{2}+y^2\right)^{3/2}} \Delta x \\[6pt]
...
\noindent To simplify further we can use the Fundamental theorem of Calculus (FTC):
\begin{equation}
    \int\limits_{a}^{b}\dfrac{df}{dx} = f(b)-f(a)
\end{equation}

\noindent Using the logic in (11) into the infinite rod: 
\begin{align*}
...
\noindent Finally, the magnitude of $\vec{E}$ due to an uniformly charged rof that is infinite in length is:
\begin{align*}
